...
export const Variables: FunctionalComponent<VariablesProps> = () => (
  ...
  --color-error-contrast: #ffdfdf;
    ...
    <h5>Base Size Variables</h5>
    <p>
      The root size variable should be the default font height for your site. The standard recommended height is 16px.
      This size is then used to scale font size, fill size (border, padding, margin), border radius, and (in the future)
      box shadows.
    </p>
    <Example>
      {`:root {
  ...
  --font-size-down-5: 0.5em;
    ...
    <h5>Font Weight</h5>
    <p>
      Font weights are a mixed bag. If a custom font is being used, not all font weights may be available. Thus, if you
      are using a custom font it is best to seed these properties with the font weights that are accessible. Most
      default browser fonts come with only normal (400) and bold (700) weights, so keep this in mind if your font weight
      classes aren't reflected correctly.
    </p>
    <Example>{`:root {
